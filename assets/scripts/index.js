...
class PullLocalWeather {
   pullCast = () => {
      ...
      weatherSummary.insertAdjacentHTML(
         "beforeend",
         `<h2>Fetching Forecast Data...</h2>`
      ); //indicate to the user that we're fetching the data
      fetch(hourlyRequestUrl) //fetch the requestURL that's been preconfigured to pull the expected weather grid
         ...
         .then((data) => {
            ...
            if (!useCachedForecast) {
               ...
               //or in the case where you ARE using cachedForecast
   ...
   parseWeatherData = (data, usingCache) => {
      ...
      if (updateHours > 12) {
         //the NWS API returns the time in 24 hours, so check to see if it's the afternoon
      ...
      let updateDayOfWeek = this.getDayOfWeek(
         this.getNumericalDate(updateTime)
      ); //get the day of the week e.g. Sun or Mon
      //console.log(updateTime[0].substring(5));
      //console.log(updateDate)

      lastUpdated.innerHTML = `<p>Data Updated @ ${updateHours}:${updateMinutes} ${amPM} </p>`;
      if (usingCache) {
         lastUpdated.insertAdjacentHTML(
            "beforeend",
            `<h2><b>Warning: Using Cached Forecast due to API Issues</b></h2>`
         );
      }
      ...
      periods.forEach((period) => {
         ...
         if (period.dewpoint !== null) {
            //check to ensure that the dewpoint value is not null. ONly try to create an add an object when it is not null
            ...
            currentDayBeingTabulated = this.getDayOfWeek(
               this.getNumericalDate(entryTime)
            );
            //check to see if the day has changed since the last period
            if (
               currentDayBeingTabulated !== lastDayTabulated &&
               periodIndex !== 0
            ) {
               const newDailyBlueHourCount = {
                  //if so, then log the count of blue hours from the previous day to a new object
               ...
               if (dailyBlueHours === 0) {
                  //then the day we just finished tabulated contained 0 blue hours so we need to put a dummy object on the graph
                  let obj = {
                     x: lastDayEntryTime,
                     y: 9,
                     r: 0,
            ...
            if (
               (windSpeed < windThreshold &&
                  tempDewDelta > dewDeltaThreshold) ||
               (period.temperature > temperatureThreshold &&
                  f < dewThreshold) ||
               tempDewDelta > 10
            ) {
               goodConditions = true;
               ...
               if (streakOfBlueHours > longestBlueHourStreak) {
                  dayWithLongestBlueStreak = this.getDayOfWeek(
                     this.getNumericalDate(entryTime)
                  );
            ...
            if (windSpeed < 10) {
               //insert a leading zero for a wind < 10
      ...
      this.printSummary(
         countOfBlueHours,
         dayWithLongestBlueStreak,
         longestBlueHourStreak,
         totalHours
      );
   ...
   getYear = () => {
      //run this method to get the year expected in createDateObject
      return new Date().getFullYear();
   };

   getNumericalDate = (timeArray) => {
      //takes in the split entry time value which is mapped into an array. We take the first 5 character of the first index value of that array and
      return timeArray[0].substring(5); //e.g. returns 08-13 on August 13
   };

   createDateObject = (numericalDate, currentYear) => {
      //expects a Numerical Date e.g. 08-12 and currentYear e.g. 2023.
      return new Date(currentYear + "-" + numericalDate);
   };

   getTimezone = (dateObj) => {
      // expects a Date Object input e.g.: Wed Aug 16 2023 17:00:00 GMT-0700 (Pacific Daylight Time)
      //console.log(dateObj)
      return dateObj.getTimezoneOffset();
   };

   getDayOfWeek = (numericalDate) => {
      //getDayOfWeek expects a 5 character string e.g. 08-27 for August 27th and returns e.g. Wed for Wednesday
   ...
   printSummary = (
      ...
      if (countOfBlueHours > 0) {
         weatherSummary.insertAdjacentHTML(
            ...
            <div class="forecast-summary-entry"><u>Best Looking Day:</u> <br/> On <b>${dayWithLongestStreak}</b> there will be a streak of <b>${longestStreakCount} blue ${hourOrHours}!</b></div>
         ...
         weatherSummary.insertAdjacentHTML(
            "beforeend",
            `<li>There are ZERO blue hours in the forecast!</li>`
         );
      }
   };
   ...
   createChart = (
      ...
      //console.log(dailySummaryDetail)
      const weatherBubbleChart = document.getElementById(
            ...
            scales: {
               ...
               x: {
                  ...
                  ticks: {
                     stepSize: 1,
                     callback: function (val) {
                        //and modify the ticks such that we print e.g. Sun, Mon etc
                        //console.log(val)
